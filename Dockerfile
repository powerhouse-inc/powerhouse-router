...
ENV SKY_ATLAS_SWITCHBOARD_STAGING=https://sky-atlas-switchboard-staging-034d973f33da.herokuapp.com
ENV SKY_ATLAS_CONNECT_STAGING=https://sky-atlas-connect-staging-ba28faf23a9f.herokuapp.com

ENV SKY_ATLAS_SWITCHBOARD=https://sky-atlas-demo-switchboard-cc29665c14ff.herokuapp.com
ENV SKY_ATLAS_CONNECT=https://sky-atlas-demo-connect-c09a7081a878.herokuapp.com
ENV SETUP_SCRIPT_PATH=https://raw.githubusercontent.com/powerhouse-inc/powerhouse/refs/heads/main/packages/ph-cli/scripts/setup.sh
COPY nginx.conf /etc/nginx/conf.d/default.conf.template
CMD /bin/sh -c "envsubst '\$PORT,\$POWERHOUSE_SWITCHBOARD,\$MAKERDAO_SWITCHBOARD,\$ARBITRUM_SWITCHBOARD,\$POWERHOUSE_SWITCHBOARD_STAGING,\$MAKERDAO_SWITCHBOARD_STAGING,\$ARBITRUM_SWITCHBOARD_STAGING,\$POWERHOUSE_CONNECT,\$MAKERDAO_CONNECT,\$ARBITRUM_CONNECT,\$POWERHOUSE_CONNECT_STAGING,\$MAKERDAO_CONNECT_STAGING,\$ARBITRUM_CONNECT_STAGING,\$RENOWN,\$RENOWN_STAGING,\$FRONTPAGE,\$POWERHOUSE_ CONNECT_DEVELOP,\$POWERHOUSE_SWITCHBOARD_DEVELOP,\$PH_WEBSOCKETS_SWITCHBOARD,\$PH_WEBSOCKETS_CONNECT,\$POWERHOUSE_CONNECT_DEVELOP,\$SKY_ATLAS_SWITCHBOARD_STAGING,\$SKY_ATLAS_CONNECT_STAGING,\$SETUP_SCRIPT_PATH,\$SKY_ATLAS_SWITCHBOARD,\$SKY_ATLAS_CONNECT' < /etc/nginx/conf.d/default.conf.template > /etc/nginx/nginx.conf" && nginx -g 'daemon off;'
